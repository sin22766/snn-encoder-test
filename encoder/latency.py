...
class BurstEncoder(SpikeEncoder):
    ...
    def encode(self, x: torch.Tensor) -> torch.Tensor:
        ...
        padding = -time_steps % self.max_window

        padded_x = torch.nn.functional.pad(x, (0, padding), value=float("nan"))

        downsampled_x = padded_x.reshape(batch * channels * freqs, -1, self.max_window).nanmean(dim=-1)

        spike_index = torch.arange(0, self.max_window, 1).expand((batch * channels * freqs, downsampled_x.shape[1], self.max_window))

        spike_count = torch.ceil(downsampled_x * self.n_max)
        isi = torch.ceil(self.t_max - (downsampled_x * (self.t_max - self.t_min)))
        ...
        spike_train = spike_train.reshape(batch, channels, freqs, downsampled_x.shape[1] * self.max_window)
    ...
    def decode(self, x: torch.Tensor) -> torch.Tensor:
        ...
        time_steps = time_steps // self.max_window

        x_reshaped = x.reshape(batch * channels * freqs, time_steps, self.max_window)

        decoded = torch.sum(x_reshaped, dim=-1) / self.max_window
        decoded = decoded.repeat_interleave(self.max_window, dim=-1)
        decoded = decoded.reshape(batch, channels, freqs, time_steps * self.max_window)
...
class BurstEncoderExpand(SpikeEncoder):
    ...
    def encode(self, x: torch.Tensor) -> torch.Tensor:
        ...
        spike_index = torch.arange(0, self.max_window, 1).expand((batch * channels * freqs, time_steps, self.max_window))

        spike_count = torch.ceil(x_reshaped * self.n_max)
        isi = torch.ceil(self.t_max - (x_reshaped * (self.t_max - self.t_min)))
        burst_length = spike_count * (isi + 1)

        spike_train = (spike_index % (isi.unsqueeze(-1) + 1) == 0).float()
        spike_train[spike_index >= burst_length.unsqueeze(-1)] = 0
        spike_train = spike_train.reshape(batch, channels, freqs, time_steps * self.max_window)
