...
class LitSTFTFinalSeizureClassifier(L.LightningModule):
    def __init__(
        self,
        model_config: ModelConfig,
        optimizer_config: OptimizerConfig,
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Compute the loss
        loss = self.criterion(spk_rec, targets)
        accuracy = spike_rate_accuracy(spk_rec, targets)
        f1 = spike_rate_f1(spk_rec, targets)
    ...
    def test_step(self, batch, batch_idx):
        ...
        loss = self.criterion(spk_rec, targets)
        accuracy = spike_rate_accuracy(spk_rec, targets)
        precision = spike_rate_precision(spk_rec, targets)
        recall = spike_rate_recall(spk_rec, targets)
        f1 = spike_rate_f1(spk_rec, targets)
        mse = spike_rate_mse(spk_rec, targets)
        total_input_spikes = data_spike.sum()
        ...
        self.log("test_total_spikes", total_input_spikes, on_step=False, on_epoch=True, prog_bar=True)
