...
class ModelConfig(TypedDict):
    """
    Configuration parameters for the SNN seizure classifier

    Attributes:
        threshold (float): Neuron firing threshold
        slope (float): Slope parameter for surrogate gradient
        beta (float): Membrane potential decay rate
        dropout_rate1 (float): Dropout rate for first fully connected layer
        dropout_rate2 (float): Dropout rate for output layer
    """
...
class EEGSpikeClassifier(nn.Module):
    """
    ...
    def __init__(
        ...
        """
        Initialize the SNN classifier for seizure detection.

        Args:
            config: Configuration parameters for the model
            input_channels: Number of EEG channels (default: 22)
            freq_dim: Frequency dimension of the STFT features (default: 129)
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Args:
            x: Input tensor of shape (batch, channels, freq, time)
                where channels is the number of EEG channels,
                freq is the frequency dimension from STFT,
                and time is the number of time steps.

        Returns:
            Tuple containing:
                - Stack of output spikes across all time steps
                - Stack of membrane potentials across all time steps
        """
        time_steps = x.size(3)
...
class AltEEGSpikeClassifier(nn.Module):
    """
    Spiking Neural Network for EEG-based seizure classification using STFT features.

    This model processes multi-channel EEG data and classifies each sequence as seizure or non-seizure.

    This is the alternative version of the EEGSpikeClassifier, which process only the magnitude part of the STFT output.
    Instead of using only the magnitude of the complex STFT value, this will use tuple of magnitude and phase.
    """
